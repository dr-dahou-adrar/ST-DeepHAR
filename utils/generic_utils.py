...
#''' The function responsible for Loading and Reading Training and Testing data save inform of npy format'''
def load_dataset_at(index, fold_index=None, normalize_timeseries=False, verbose=True) -> (np.array, np.array):
    ...
    # '''Bring out the activty class labels Y and then peform normalization to set the value in range of [0 - (MAX - 1)]'''
    nb_classes = len(np.unique(y_train))
    y_train = (y_train - y_train.min()) / (y_train.max() - y_train.min()) * (nb_classes - 1)

    if is_timeseries:
        # scale the elements to the specified range
        if normalize_timeseries:
            X_train_mean = X_train.mean()
            X_train_std = X_train.std()
            X_train = (X_train - X_train_mean) / (X_train_std + 1e-8)

    if verbose: print(" Completed preparing the training data...")
    ...
    if verbose:
        print("Completed preparing the testing data...")
        print()
        print("The count of the training instances : ", X_train.shape[0], "The count of the training instances of the testing instances : ", X_test.shape[0])
        print("Number of the activty class labels : ", nb_classes)
...
def cutoff_sequence(X_train, X_test, choice, dataset_id, sequence_length):
    assert MAX_NB_VARIABLES[dataset_id] < sequence_length, "when the sequence is going to be tremmed, maximum sequence" \
                                                                   " the length should be shorter than the original sequence length."
    ...
    if choice == 'pre':
        ...
        if X_test is not None:
            X_test = X_test[:, :, :-cutoff]
    print("The new sequence lengths :", MAX_NB_VARIABLES[dataset_id])
