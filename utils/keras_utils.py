...
def _average_gradient_norm(model, X_train, y_train, batch_size):
    # we first invetigate whether the model(i.e.ST-DeepHAR) was already compiled or not
    if not hasattr(model, "train_function"):
        raise RuntimeError("You must compile your model before using it.")

    weights = model.trainable_weights  # the tensors of weights
    ...
    input_tensors = [
        # input sequences
        model.inputs[0],
        # the amount by which the sample should be weighted
        model.sample_weights[0],
        # activity class labels
        model.targets[0],
        # determine whether the model in the training or testing phase
        K.learning_phase()
    ]
    ...
    while steps < nb_steps:
        X, y = next(datagen)
        # for every input assign one to the sample weights 
        if s_w is None:
